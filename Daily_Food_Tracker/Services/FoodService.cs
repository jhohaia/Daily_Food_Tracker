...
namespace Daily_Food_Tracker.Services
{
    public static class FoodService
    {
        ...
        static async Task Init()
        {
            if (db != null) return;
            //
            var databasePath = Path.Combine(FileSystem.AppDataDirectory, "foodsql.db");

            db = new SQLiteAsyncConnection(databasePath);
            await db.CreateTableAsync<Food>();
        }

        public static async Task AddFood(string foodid, string foodname, string calories)
            //string alcohol, 
            //string carbohydrates, string calcium, 
            //string cholesterol, string calories, 
            //string kilojule, string fat, string glucose, 
            //string iron, string lactose, 
            //string magnesium, string potassium, 
            //string selenium, string sodium,
            //string starch, string sugars, 
            //string water, string zinc)
